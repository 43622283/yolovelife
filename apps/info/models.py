...
class ENVIRTUAL(models.Model):
    ...
    class Meta:
        permissions = (
            ('deveops_api_list_envirtual', u'罗列部署地点'),
            ('deveops_api_create_envirtual', u'创建部署地点'),
            ('deveops_api_delete_envirtual', u'删除部署地点'),
        )
...
class INFO(models.Model):
    ...
    name = models.CharField(max_length=6000, default='无', null=True, blank=True)

    detail = models.CharField(max_length=6000, default='无', null=True, blank=True)

    main_manager = models.ForeignKey(ExtendUser, related_name='infos_A', null=True, on_delete=models.SET_NULL)

    managers = models.ManyToManyField(ExtendUser,  related_name='infos', blank=True, null=True)

    developers = models.ManyToManyField(DEVELOPER, related_name='infos', blank=True, null=True)
